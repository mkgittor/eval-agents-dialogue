...
import click
from .bloombergfinance import BloombergFinancialNewsDataset
...
DEFAULT_DATASET_NAME = "BloombergFinancialNews-Subset"


async def upload_bloomberg_finance_to_langfuse(
    dataset_name: str,
    samples: int = 10,
    category: str | None = None,
    ids: list[int] | None = None,
) -> None:
    """Upload BloombergFinancialNews examples to Langfuse.

    This function converts BloombergFinancialNews examples to a temporary JSONL file
    ...
    """
    # Load BloombergFinancialNews dataset
    logger.info("Loading BloombergFinancialNews dataset...")
    ...
    with tempfile.NamedTemporaryFile(
        ...
        encoding="utf-8",
        suffix=".jsonl",
        prefix=f"bloombergfinance_{dataset_name}_",
        delete=False,
    ) as temp_file:
        temp_path = Path(temp_file.name)
        logger.info(f"Writing {len(dataset)} examples to temporary file...")
...
def cli(dataset_name: str, samples: int, category: str | None, ids: tuple[int, ...]) -> None:
    ...
    asyncio.run(upload_bloomberg_finance_to_langfuse(dataset_name, samples, category, ids_list))
